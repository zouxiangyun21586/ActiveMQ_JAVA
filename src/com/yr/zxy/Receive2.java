...
public class Receive2 {
	...
	// 连接地址
	private String brokerURL = "tcp://192.168.1.237:61616?jms.prefetchPolicy.all=2";
	// connection的工厂
	...
	public static void main(String[] args) {
		Receive2 receive = new Receive2();
		receive.start();
		receive.getTextMessage(); // 取 String
//		receive.getObject();  // 取 对象
//		receive.getMapMessage(); // 取 Map
//		receive.getByteMessage(); // 取 Byte
			...
			consumer.setMessageListener(new MessageListener() {
				public void onMessage(Message message) {
					try {
						...
						String name = map.getString("name"); // 获取值
			...
			consumer.setMessageListener(new MessageListener() {
				...
				public void onMessage(Message message) {
					...
					try {
						Thread.sleep(100);
						// 获取到接收的数据
						String text = ((StreamMessage) message).toString();
						System.out.println("接受消息:" + text);
					} catch (Exception e) {
						e.printStackTrace();
					}
			...
			consumer.setMessageListener(new MessageListener() {
				public void onMessage(Message message) {
					...
					try {
						// 获取到接收的数据
						BytesMessage bytesMessage = (BytesMessage) message;
						out = new FileOutputStream("G:/xm/aa.jpg"); // 文件保存路径
						byte[] byt = new byte[10086];
						int len = 0;
						while ((len = bytesMessage.readBytes(byt)) != -1) {
							out.write(byt, 0, len);
						}
						out.close(); // 必须关闭流 否则一直在写数据进去 文件中会显示 0KB
						System.out.println("写入文件成功");
					} catch (Exception e) {
						e.printStackTrace();
					}
